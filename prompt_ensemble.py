...
def dbscan(text_features, dataset, device):
    # 将输入张量转换为NumPy数组
    data = text_features.permute(1,0,2).detach().cpu().numpy()
    ...
    for i in range(2):
        ...
        if len(labels) > 0:
            label_counts = np.bincount(labels[labels >= 0])  # 计算每个簇中数据点的数量
            if len(label_counts) > 0:
                label = np.argmax(label_counts)  # 获取具有最多数据点的标签
                class_embedding = np.mean(data[i][labels == label], axis=0)  # 计算每个簇的平均值，即类别的聚类中心
            else:
                # 处理 label_counts 为空的情况
                class_embedding = np.mean(data[i], axis=0)
        else:
            # 处理 labels 为空的情况
            label = 0
            class_embedding = np.mean(data[i], axis=0)

        labels_list.append(torch.from_numpy(labels).unsqueeze(0))  # 转换标签为张量并添加到列表中
        class_embedding_list.append(torch.from_numpy(class_embedding).unsqueeze(0))  # 转换聚类中心为张量并添加到列表中

    # 合并聚类中心张量
    class_embeddings = torch.cat(class_embedding_list, dim=0).clone().detach().unsqueeze(0).to(device)
    return class_embeddings  # 返回标签张量和聚类中心张量
...
class AnomalyCLIP_PromptLearner(nn.Module):
    def __init__(self, clip_model, design_details):
        ...
        # scale = 1024 ** -0.5
        # self.proj = nn.Parameter(scale * torch.randn(1370,12))
    ...
    def forward(self, cls_id =None):
        ...
        ctx_neg = self.ctx_neg#1,1,12,768
        ctx_pos = self.ctx_pos
        ctx_neg = self.ctx_neg

        # bias = feature.permute(0, 2, 1) @ self.proj
        # bias = bias.permute(0, 2, 1).unsqueeze(0)
        ...
        _, l, d = self.tokenized_prompts_pos.shape#[1,1,77]
        ...
        tokenized_prompts = torch.cat((tokenized_prompts_pos, tokenized_prompts_neg), dim = 0)#[2,77]

        
        return prompts, tokenized_prompts, self.compound_prompts_text
    
class CLIP_PromptLearner(nn.Module):
    def __init__(self, clip_model, design_details, dataset = "", mode = "train"):
        super().__init__()
        if mode == "train":
           if dataset == "mvtec":
            ...
            #    classnames = ["bottle cable capsule carpet grid hazelnut leather metal_nut pill screw tile toothbrush transistor wood zipper"]
               ...
               classnames = ["candle", "capsules", "cashew", "chewinggum", "fryum", "macaroni1", "macaroni2", "pcb1", "pcb2", "pcb3", "pcb4", "pipe_fryum", "object", "object", "object"]
            #    classnames = ["candle capsules cashew chewinggum fryum macaroni1 macaroni2 pcb1 pcb2 pcb3 pcb4 pipe_fryum"]
        else:
            classnames = ["object","object","object","object","object","object","object","object","object","object","object","object","object","object","object",]    
        # classnames = ["bottle cable capsule carpet grid hazelnut leather metal_nut pill screw tile toothbrush transistor wood zipper candle capsules cashew chewinggum fryum macaroni1 macaroni2 pcb1 pcb2 pcb3 pcb4 pipe_fryum"]
        self.n_cls = len(classnames)
        self.n_ctx = design_details["Prompt_length"]#12
        n_ctx_pos = self.n_ctx
        n_ctx_neg = self.n_ctx
        ...
        with torch.no_grad():
            embedding_pos = clip_model.token_embedding(tokenized_prompts_pos).type(dtype)
            embedding_neg = clip_model.token_embedding(tokenized_prompts_neg).type(dtype)
            # embedding_pos = embedding_pos.mean(dim=0).unsqueeze(0)#new
            # embedding_neg = embedding_neg.mean(dim=0).unsqueeze(0)#new
        ...
        # tokenized_prompts_pos = tokenized_prompts_pos.float().mean(0).unsqueeze(0)#new
        # tokenized_prompts_neg = tokenized_prompts_neg.float().mean(0).unsqueeze(0)#new
...
class AnomalyCLIP_PromptLearners(nn.Module):
    def __init__(self, clip_model, design_details):
        super().__init__()
        classnames = ["category"]
        self.n_cls = len(classnames)
        ...
        if ctx_init_pos and ctx_init_neg:
            ...
            if True:
                print("Initializing class-specific contexts")
                #这里是cls是类的个数，n_ctx_pos代表learnable token的长度，ctx_dim表示prompt的dimension
                ctx_vectors_pos = torch.empty(self.normal_num, n_ctx_pos, ctx_dim, dtype=dtype)
                ctx_vectors_neg = torch.empty(self.anormaly_num, n_ctx_neg, ctx_dim, dtype=dtype)
            else:
                print("Initializing a generic context")
                ctx_vectors_pos = torch.empty(n_ctx_pos, ctx_dim, dtype=dtype)
                ctx_vectors_neg = torch.empty(n_ctx_neg, ctx_dim, dtype=dtype)
    ...
    def forward(self, cls_id =None,feature=None):
...
class CLIP_PromptLearners(nn.Module):
    def __init__(self, clip_model, design_details):
        super().__init__()
        classnames = ["bottle", "cable", "capsule", "carpet", "grid", "hazelnut", "leather", "metal_nut", "pill", "screw", "tile", "toothbrush", 
                      "transistor", "wood", "zipper","candle", "capsules", "cashew", "chewinggum", "fryum", "macaroni1", 
                      "macaroni2", "pcb1", "pcb2", "pcb3", "pcb4", "pipe_fryum", "object", "object", "object"]
