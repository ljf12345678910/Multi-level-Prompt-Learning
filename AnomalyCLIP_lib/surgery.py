...
def clip_feature_surgerys(image_features, text_features, redundant_feats=None, t=2):
        similarity = []
        #################### multi-layers fusion ####################
        
            
        text_feature = text_features.permute(0, 2, 1)
        #print(image_feature)
        #print(image_feature.shape)#[16,226,640]
        #print(text_feature)#[16,640,2]
        #print(text_feature.shape)
        prob = image_features[:, :1, :] @ text_feature  # 8, 1, 2
        prob = prob.softmax(-1)  # 8, 1, 2
        # prob = (prob * 2).softmax(-1)
        mean_prob = prob.mean(-1, keepdim=True)
        if (mean_prob == 0).any():
            mean_prob += 1e-8  # 添加一个小的正数以避免除零错误
        w = prob / mean_prob
        b, n_t, n_i, c ,b2= image_features.shape[0], text_features.shape[1], image_features.shape[1], image_features.shape[2], text_features.shape[0]
        feats = image_features.reshape(b, n_i, 1, c) * text_feature.permute(0, 2, 1).contiguous().reshape(b2, 1, n_t, c)
        ...
        return (similarity_tmp/0.07).softmax(-1)
def clips_feature_surgery(image_features, text_features, redundant_feats=None, t=2):
        similarity = []
        #################### multi-layers fusion ####################
        for idx in range(len(image_features)):
            image_feature = image_features[idx]
            text_feature = text_features[idx].unsqueeze(0).permute(0, 2, 1)
            #print(image_feature)
            #print(image_feature.shape)#[16,226,640]
            #print(text_feature)#[16,640,2]
            #print(text_feature.shape)
            ...
            b, n_t, n_i, c = image_feature.shape[0], text_feature.shape[2], image_feature.shape[1], image_feature.shape[
                2]
            feats = image_feature.reshape(b, n_i, 1, c) * text_feature.permute(0, 2, 1).contiguous().reshape(1, 1, n_t,
                                                                                                             c)
            feats *= w.reshape(b, 1, n_t, 1)
            redundant_feats = feats.mean(2, keepdim=True)  # along cls dim
            feats = feats - redundant_feats
