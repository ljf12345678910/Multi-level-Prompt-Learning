...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        ...
        x = x + pos
        #visual_ctx = visual.expand(x.shape[0], -1, -1).half()
        ...
        if True:
            ...
            for patch_token in patch_tokens:
                ...
                #if True:
                   # if not (self.counter > len(compound_prompts[i]) - 1):
                       # x = patch_token.permute(1, 0, 2)
                      #  prefix = x[0:x.shape[0] - self.compound_prompt_nctx, :, :]
                        # Create/configure learnable tokens of this layer
                      #  visual_context = compound_prompts[i][self.counter]  # extract the correct index
                      #  visual_context = visual_context.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                       ...
                       # i += 1
                       # self.counter += 1
                       # patch_token = x.permute(1, 0, 2)
                patch_token_list.append(patch_token)
            patch_tokens = patch_token_list
            
            return x_ori[0, :, :] @ self.proj, patch_tokens,x @ self.proj
