...
def _download(
    ...
    if not cache_dir:
        ...
        cache_dir = os.path.expanduser("/home/ljf/.cache/clip")
...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", design_details = None, jit: bool = False, download_root: str = None):
    ...
    if name in _MODELS:
        ...
        model_path = _download(_MODELS[name], download_root or os.path.expanduser("/home/ljf/.cache/clip"))
...
def compute_similaritys(image_features, text_features):
    prob_1 = image_features[:, :1, :] @ text_features.permute(0,2,1)
    b, n_t, n_i, c ,b2= image_features.shape[0], text_features.shape[1], image_features.shape[1], image_features.shape[2], text_features.shape[0]#8,2,1370,768
    feats = image_features.reshape(b, n_i, 1, c) * text_features.reshape(b2, 1, n_t, c)
    similarity = feats.sum(-1)
    return (similarity/0.07).softmax(-1), prob_1
